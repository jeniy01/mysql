...
-- 기 생성된 사용자의 비밀번호 수정
SET PASSWORD FOR 'test2'@localhost = PASSWORD('a1234');

-- DB 권한 부여
GRANT ALL PRIVILEGES ON *.* to 'test2'@'localhost';

-- 특정 DB에만 권한 부여
-- GRANT ALL PRIVILEGES ON company.* to 'test2'@'localhost'; 

-- 특정 DB에 select 권한만 주기
-- GRANT SELECT ON company.* TO 'test2'@'localhost' IDENTIFIED BY 'a1234'; 
...
-- 사용자 권한 회수
REVOKE ALL PRIVILEGES ON *.* FROM 'test2'@'localhost';
